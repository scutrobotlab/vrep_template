...
typedef struct 
{
  ...
  /* Coefficients for different gears(Max_gear_speed = Coefficients*joint_max_speed).(Between 0 and 1) */
  ...
  float STEER_RANGE;
	...
	float WHEEL_RADIUS;
  /* Time interval for controller. */
	float RUN_INTERVAL;	
  int16_t MAX_MOTOR_SPEED;
...
class CChassis
{
  private:
    ...
    uint8_t joint_control();
    uint8_t acce_optimization();
    uint8_t attitude_protect();
    ...
    int32_t*(*legWheel_controller)(const _chassis_GlobalPos* fb_pos, const int32_t* fb_joint_ang, const _chassis_Velocity* cmd_vel);
#if defined(SWERVE_CHASSIS)
    void steer_angle_convert();
#endif
  public:
    _chassis_GlobalPos   Zero_Pos;                 //!<底盘零位姿点
    _chassis_GlobalPos   Command_Pos;              //!<底盘指令位姿
    _chassis_Velocity    Command_Velocity;         //!<底盘指令速度矢量
    _chassis_Velocity    Target_Velocity;          //!<优化后底盘目标速度矢量
    _chassis_GlobalPos   Current_Pos;              //!<底盘当前位姿
    int32_t              joint_angle[JOINT_NUM];   //!<关节当前角度
    int16_t              joint_rpm[JOINT_NUM];     //!<关节当前转速
    int16_t              joint_torque[JOINT_NUM];  //!<关节当前转矩（或电流）
    int16_t              joint_rpmOut[JOINT_NUM];  //!<解算后关节转速
    int32_t              joint_Out[JOINT_NUM];     //!<控制器作用后关节输出
#if defined(SWERVE_CHASSIS)
    _Steer_Type          steer_Set[STEER_NUM];     //!<轮组角度信息
#endif

    /* 设置初始参数 */
    CChassis( float chassis_wheel_track, float chassis_wheel_base, float wheel_radius,\
							float max_motor_output, int16_t max_motor_speed,\
      ...
      Param.RUN_INTERVAL        = run_interval;
      Param.MAX_MOTOR_SPEED     = max_motor_speed;
			Param.launch_speed        = Param.MAX_MOTOR_SPEED;
      Param.acceOptimize_flag     = optimize_flag;
    ...
    */
    ...
    void Load_SpeedController(int32_t*(*pFunc)(const int16_t*, const int16_t*));   
		void Load_LegWheelController(int32_t*(*pFunc)(const _chassis_GlobalPos*, const int32_t*, const _chassis_Velocity*));
